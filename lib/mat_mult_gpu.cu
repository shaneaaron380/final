...
__global__ void MatMultKernel(const Matrix A, const Matrix B, const float alpha, const int N)
{
  int l = 0;
...
__global__ void MatMultKernelShared(const Matrix A, const Matrix B, const float alpha, const int N)
{
	int l = 0, k = 0;
	int j = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    //if (t_idx == 241) cuPrintf("j=%d, C.els[j]=%f, B.els[j]=%f\n", j, C.els[j], B.els[j]);
		for (int i = 1; i < N; i++) {
			...
			while (k < i)  {
				...
				__syncthreads();
				S -= As[0] * B.els[k*N+j]; //S -= A[i][k] * C[k][j];
